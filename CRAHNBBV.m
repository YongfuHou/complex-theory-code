...
M=25;
C_state=zeros(1,M); %M个信道的初始化状态，规定状态0为信道空闲，状态1信道被主用户占用
...
for ttt=1:T
    ...
    while (flag || tab)
        ...
        j=sel-(i-1)*10;
        %确定该网格横纵坐标范围
        X_center=(L/2+(j-1)*L);%确定网格中心点坐标%%%%7.4
        Y_center=(L/2+(i-1)*L);
     %确定横坐标范围
...
sum_edge=0;
for i=1:N_flag
   sum_edge=sum_edge+sum(Edge(i,:));
end
%% -----------------绘制网络拓扑图------------------ %%
% figure(aaa)
% axis([0 inf 0 inf]);
% plot(X_Node,Y_Node,'ro','MarkerEdgeColor','g','MarkerFaceColor','r','markersize',8);
% hold on;
% for i=1:(node_num-1) 
%      for j=i+1:node_num
...
% eval(['save S',num2str(Pn)]); %%%保存每次循环的点强分布
% saveas(gca,num2str(T),'jpg');   %%%保存每次循环产生的图片
% close(gcf);
% node_n(1,Pn+1)= node_num;    %%%%  记录每次剩余的节点数
%  edge_n(1,Pn+1)=sum_edge;
%eval(['save  Edge_f',num2str(Pn)]); %%%保存每次循环的连接矩阵
